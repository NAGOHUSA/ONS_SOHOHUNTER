...
from fetch_lasco import fetch_window as fetch_lasco_window
from fetch_ccor1 import fetch_window as fetch_ccor1_window   # <-- NEW
...
def crop_for_track(
    ...
    cropped_seq = [img[y0:y1+1, x0:x1+1].copy() for img in aligned_imgs]
    rep_crop = cropped_seq[mid_idx].copy()

    # Dynamic crop size per detector
    target = (
        CROP_SIZE_C2 if det == "C2" else
        CROP_SIZE_C3 if det == "C3" else
        112  # CCOR1: 112 px ≈ same angular scale as C2
    )
    ...
    if rep_crop_resized.shape[:2] != rep_crop.shape[:2]:
        s = float(rep_crop_resized.shape[1]) / float(rep_crop.shape[1])
    for (t,x,y,a) in tr:
        x_local = (x - x0) * s
        y_local = (y - y0) * s
    ...
    gif_ok = write_gif(seq8, gifs_dir/gif_name, fps=GIF_FPS) if imageio and len(seq8) >= 2 else False
    mp4_ok = write_mp4(seq8, mp4_dir/mp4_name, fps=MP4_FPS) if len(seq8) >= 2 else False
    ...
    try:
        ...
        print(f"[warn] original mid-frame copy failed: {e}")
...
def main():
    ...
    fetched_lasco = fetch_lasco_window(hours_back=args.hours, step_min=args.step_min, root="frames")
    fetched_ccor1 = fetch_ccor1_window(hours_back=args.hours, step_min=args.step_min, root="frames")
    print(f"[fetch] LASCO new: {len(fetched_lasco)}  CCOR1 new: {len(fetched_ccor1)}")

    series_c2 = load_series(pathlib.Path("frames") / "C2")
    series_c3 = load_series(pathlib.Path("frames") / "C3")
    series_ccor1 = load_series(pathlib.Path("frames") / "CCOR1")   # <-- NEW

    print(f"[DEBUG] C2: {len(series_c2)}  C3: {len(series_c3)}  CCOR1: {len(series_ccor1)}")
    ...
    def process(det: str, series):
        ...
        detectors_stats[det] = {
            "frames": len(series),
            "tracks": len(tracks),
            "last_frame_name": names[-1],
            "last_frame_iso": parse_frame_iso(names[-1]) or "",
    ...
    all_hits.extend(process("CCOR1", series_ccor1))   # <-- NEW
    ...
    summary = {
        ...
        "fetched_new_frames": len(fetched_lasco) + len(fetched_ccor1),
        "errors": [],
    ...
    if all_hits:
        ...
        with open(out_dir/f"candidates_{ts_name}.json","w",encoding="utf-8") as f:
            json.dump(all_hits, f, indent=2)
        with open(out_dir/"candidates_latest.json","w",encoding="utf-8") as f:
            json.dump(all_hits, f, indent=2)
    else:
