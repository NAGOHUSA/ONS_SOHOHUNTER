...
detect_comets.py
----------------
Download LASCO C2/C3 frames, run a lightweight moving-point detector + AI classifier + Groq API (limited 2 calls/12h),
write:
    ...
    for idx, (path, ts) in enumerate(paired):
        ...
        for kf, pos, age, tid in active_kfs[:]:
            if age == 0 and len([p for p in paired[: idx + 1] if p[1] <= ts]) >= 4:
                ...
                if cls["score"] >= 0.5:      # comet confidence
                    candidates.append({
                        "instrument": instr,
                        "timestamp": ts,
                        "track_id": tid,
...
# ----------------------------------------------------------------------
# GROQ API ADVANCED CLASSIFICATION (LIMITED)
# ----------------------------------------------------------------------
def advanced_groq_classify(candidates):
    if not GROQ_API_KEY:
        log("No GROQ_API_KEY – skipping advanced AI")
        return candidates

    # Check last call time
    if LAST_CALL_FILE.exists():
        last_call = datetime.fromtimestamp(float(LAST_CALL_FILE.read_text().strip()))
        if datetime.utcnow() - last_call < timedelta(hours=GROQ_LIMIT_HOURS):
            log("Groq limit reached – skipping")
            return candidates
    else:
        LAST_CALL_FILE.parent.mkdir(parents=True, exist_ok=True)

    # Select 2 highest ai_score >0.5
    high_score = sorted([c for c in candidates if c["ai_score"] > 0.5], key=lambda c: c["ai_score"], reverse=True)[:GROQ_MAX_CALLS]

    if not high_score:
        log("No high-score candidates for Groq")
        return candidates

    client = Groq(api_key=GROQ_API_KEY)

    for c in high_score:
        crop_path = OUT_DIR / c["crop_path"]
        ...
        try:
                ...
                messages=[
                    {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": "Analyze this SOHO LASCO crop. Is it a comet? Respond with label: comet or not_comet, score: 0-1, description: short description."},
                            {
                                "type": "image_url",
                                "image_url": {
            ...
            # Parse response (assume "label: comet, score: 0.9, description: bright streak")
            ...
            for line in lines:
                if "label" in line:
                    label = "comet" if "comet" in line else "not_comet"
                if "score" in line:
                    try:
            ...
            c["groq_label"] = label
            c["groq_score"] = score
            c["groq_description"] = description
            log(f"Groq on {c['crop_path']}: {label} ({score}) - {description}")
        except Exception as e:
            log(f"Groq error: {e}")
...
def main():
    ...
    # 5. Always write status & run proof
