...
def load_dataset():
    ...
    with open(CSV_PATH, "r", newline="") as f:
        ...
        for i, row in enumerate(rdr, start=2):
            if not row:
                print(f"[skip@line{i}] empty row"); continue
            p = (row.get("path") or "").strip()
            lab = (row.get("label") or "").strip().lower()
            if not p or not lab:
                print(f"[skip@line{i}] missing path/label"); continue
            if not os.path.exists(p):
                print(f"[skip@line{i}] not found: {p}"); continue

            im = cv2.imread(p, cv2.IMREAD_GRAYSCALE)
            if im is None:
                print(f"[skip@line{i}] cannot read: {p}"); continue
